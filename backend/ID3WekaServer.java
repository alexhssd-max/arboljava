...
async function cargarArbol() {
    try {
        ...
        // 3. Renderizar el árbol con líneas completamente derechas
...
// POSICIONAMIENTO JERÁRQUICO SIMÉTRICO: Centra al padre basándose en el bloque completo de hijos
function layoutTree(node, x, y) {
    ...
    keys.forEach((key, i) => {
        ...
        childWidths.push(w);
        totalChildrenW += w;
        if (i < keys.length - 1) totalChildrenW += H_GAP;
    });
    ...
    // ALINEACIÓN TOTALMENTE DERECHA: El padre se ubica exactamente en el centro medio geométrico de sus hijos
...
// GENERADOR DE ELEMENTOS VISUALES CON LÍNEAS ORTOGONALES (DERECHITAS EN 90 GRADOS)
function generateSVG(node, els = [], edges = []) {
    if (node.type === 'leaf') {
        ...
        Object.entries(node.children).forEach(([value, child]) => {
            ...
            // TRUCO DE LA "T" INVERTIDA: Calcula un punto intermedio en Y para que la línea baje recta, quiebre y vuelva a bajar recta
            const midY = startY + (V_GAP / 2);

            edges.push(`
                <path d="M${startX},${startY} L${startX},${midY} L${endX},${midY} L${endX},${endY}" stroke="#3a3a5c" stroke-width="2" fill="none" stroke-linejoin="round"/>
                
                <g>
                    <rect x="${endX - 30}" y="${midY + 10}" width="60" height="16" rx="4" fill="#11111b"/>
                    <text x="${endX}" y="${midY + 22}" text-anchor="middle" fill="#8c8ca8" font-size="10" font-weight="bold">${value}</text>
                </g>
            `);
...
// PINTAR EN PANTALLA: Inyecta el bloque SVG final adaptado al contenedor centrado
